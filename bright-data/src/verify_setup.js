...
if (process.env.GOOGLE_API_KEY) {
  console.log('   OK: GOOGLE_API_KEY is set (enables LLM summaries)');
} else {
  console.log('   Note: GOOGLE_API_KEY not set (heuristic summaries will be used)');
}
...
if (process.env.GOOGLE_API_KEY) {
  console.log('\n5) Testing Google GenAI API (optional)...');
  try {
    ...
    if (response.text()) {
      console.log('   OK: Google GenAI API connection successful');
      console.log(`   OK: Using model: ${process.env.GOOGLE_MODEL || 'gemini-2.5-flash'}`);
    }
  } catch (error) {
    console.log(`   ERROR: Google GenAI API error: ${error.message}`);
    console.log('   Note: This is optional; workshop works without it');
  }
} else {
  console.log('\n5) Google GenAI not configured (optional)');
