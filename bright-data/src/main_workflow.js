...
const CONTENT_LIMITS = {
  SCRAPE_PREVIEW: 100000,  // Characters to include per scraped page in synthesis
...
function getPromptForQuery(query) {
  ...
  "sources": ["https://weather.gov", "https://climatology.site"]
    ...
    return `Analyze web sources to determine most profitable crops for the specified pincode/area considering soil properties, climate, market prices, and other factors. List top 5-10 crops with approximate profitability percentages, yields, and why they're suitable. Create key value pairs in JSON format. Include key findings and cite sources with URLs. For example something like: {
  "location": "90210 California",
  "top_crops": [
    {
      "crop": "Avocado",
      "annual_profitability": "$15,000/acre",
      "yield_estimate": "5-10 tons/acre",
      "reason": "Favorable soil and climate"
    },
    ...
  ],
  "market_factors": "Local demand and export opportunities",
  "sources": ["https://usda.gov/agriculture", "https://extension.university"]
} Make it approximate`;
  } else {
    // Default soil properties
    return `Analyze web sources and create key value pairs like in json. Include key findings and cite sources with URLs. For example something like: {
  "pincode": "90210",
  "properties": {
    "pH": 6.8,
    "organicMatter": "2.5%",
...
async function runWorkflow(query, options = {}) {
  ...
  const llm = CONFIG.googleApiKey
    ? new ChatGoogleGenerativeAI({ apiKey: CONFIG.googleApiKey, model: CONFIG.googleModel, temperature: 0 })
    : null;
  ...
  const combinedContent = scrapedContent.map((item, i) =>
    `\n=== Source ${i + 1} [${item.domain}]: ${item.url} ===\n${item.content.substring(0, CONTENT_LIMITS.SCRAPE_PREVIEW)}`
  ).join('\n\n');
  ...
  let result;
  if (llm) {
    const systemPrompt = getPromptForQuery(query);
